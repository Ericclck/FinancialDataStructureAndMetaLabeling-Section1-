...
def get_volume_bar(ticks : pd.DataFrame, vol : int) -> pd.DataFrame:
    ...
    for i in range(len(ticks)):
        ...
        if curr_vol >= vol:
            ...
            volume_bars[it,2] = ticks.loc[last_tick:i+1,'price'].max()
            volume_bars[it,3] = ticks.loc[last_tick:i+1,'price'].min()
            volume_bars[it,4] = ticks.loc[i,'price']
            volume_bars[it,5] = ticks.loc[last_tick:i+1,'volume'].sum()
            last_tick = i+1
            curr_vol = 0
            it += 1
...
def get_dollar_bar(ticks : pd.DataFrame, dol : int) -> pd.DataFrame:
    ...
    for i in range(len(ticks)):
        ...
        if curr_dol >= dol:
            ...
            dollar_bars[it,2] = ticks.loc[last_tick:i+1,'price'].max()
            dollar_bars[it,3] = ticks.loc[last_tick:i+1,'price'].min()
            dollar_bars[it,4] = ticks.loc[i,'price']
            dollar_bars[it,5] = ticks.loc[last_tick:i+1,'volume'].sum()
            last_tick = i+1
            curr_dol = 0
            it += 1
...
def get_imbalance_dollar_bar(ticks : pd.DataFrame, window : int) -> pd.DataFrame:
    ...
    for i in range(1,len(ticks)):
        ...
        if abs(curr_imb) >= abs(expected_imb):
            ...
            dollar_bars[j,2] = ticks.loc[last_tick:i+1,'price'].max()
            dollar_bars[j,3] = ticks.loc[last_tick:i+1,'price'].min()
            dollar_bars[j,4] = ticks.loc[i,'price']
            dollar_bars[j,5] = ticks.loc[last_tick:i+1,'volume'].sum()
            j += 1
            t[j] = i+1-last_tick
            last_tick = i+1
            curr_imb = 0
    ...
    for i in range(1,len(ticks)):
        ...
        if run >= expected_run:
            if indices_only:
                ...
                run_bars[j,2] = ticks.loc[last_tick:i+1,'price'].max()
                run_bars[j,3] = ticks.loc[last_tick:i+1,'price'].min()
                run_bars[j,4] = ticks.loc[i,'price']
                run_bars[j,5] = ticks.loc[last_tick:i+1,'volume'].sum()
...
def get_events(ticks : pd.Series, h : int) -> pd.Series:
    """
    Get t events
    ...
    for i in range(1,len(ticks)):
        sp , sn = max(0,sp+ticks[i]-ticks[i-1]),min(0,sn+ticks[i]-ticks[i-1])
        if sp >= h:
            t_events[i] = True
            sp = 0
        elif sn <= -h:
